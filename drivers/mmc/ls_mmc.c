...
static int sdio_prepare_data(struct sdio_host* host, struct mmc_data *data)
{
...
static int sdio_prepare_dma(struct sdio_host *host, struct mmc_data *data)
{
	...
	pr_debug("data_phy_addr =  0x%lx,data->flags=%d\n",data_phy_addr,data->flags);
...
static int sdio_send_cmd(struct udevice *dev, struct mmc_cmd *cmd, struct mmc_data *data)
{
	...
	// EMMC 先准备数据，再发指令，且不用延迟
	if (!host->use_sdcard && data) {
		err = sdio_prepare_data(host, data);
		if (err) {
			return err;
		}
...
static int sdio_mmc_of_to_plat(struct udevice *dev)
{
	...
	host->use_sdcard = dev_read_bool(dev, "use-sdcard");
	cfg = &plat->cfg;
	cfg->name = dev->name;
