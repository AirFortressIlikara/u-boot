...
void mm_feature_init(struct mc_setting *mc)
{
	...
	mm_ctrl_info.table.enable_early_printf		= mc->early_print;
#ifdef DDR3_MODE
	mm_ctrl_info.table.ddr_param_store		= mc->ddr_param_store;
	...
	mm_ctrl_info.table.two_t_mode_enable		= mc->cmd_timing_mode;
	...
	mm_ctrl_info.odt.mc_dqs_odt_1cs			= mc->mc_dqs_odt;
	...
	mm_ctrl_info.paster.mc0_memsize			= (mc->info.size/1024/1024)*4;
#else
	mm_ctrl_info.paster.mc0_memsize			= (mc->info.size/1024/1024);
#endif
	mm_ctrl_info.paster.mc0_dram_type		= mc->sdram_type;
	mm_ctrl_info.paster.mc0_dimm_type		= mc->dimm_type;
	mm_ctrl_info.paster.mc0_module_type		= MC0_MODULE_TYPE;
	mm_ctrl_info.paster.mc0_cid_num			= MC0_CID_NUM;
	mm_ctrl_info.paster.mc0_ba_num			= mc->sdram_banks;
	mm_ctrl_info.paster.mc0_bg_num			= mc->sdram_bank_groups;
	mm_ctrl_info.paster.mc0_csmap			= mc->cs_map;
	mm_ctrl_info.paster.mc0_dram_width		= mc->sdram_width;
	mm_ctrl_info.paster.mc0_module_width		= MC0_MODULE_WIDTH;
	mm_ctrl_info.paster.mc0_sdram_capacity		= MC0_SDRAM_CAPACITY;
	mm_ctrl_info.paster.mc0_col_num			= mc->sdram_cols;
	mm_ctrl_info.paster.mc0_row_num			= mc->sdram_rows;
	mm_ctrl_info.paster.mc0_addr_mirror		= mc->addr_mirror;
...
static int ls_ddr_parse_dt(struct udevice *dev)
{
	...
	u32 val;
	int mc_id, ret;
	u32 sdram_rows, sdram_cols, sdram_banks, sdram_bank_groups;
	...
	// if (ret) {
	// 	printf("ddr: get soc-config-reg failed!\n");
	// 	return ret;
	// }
	...
	// if (ret) {
	// 	printf("ddr: get mc-disable-offs failed!\n");
	// 	return ret;
	// }
	...
	// if (ret) {
	// 	printf("ddr: get mc-default-offs failed!\n");
	// 	return ret;
	// }
	// mc->mc_default_offs = val;

	str = dev_read_string(dev, "sdram-type");
	if (str && !strcmp(str, "ddr4")) {
		mc->sdram_type = MC_SDRAM_TYPE_DDR4;
	...
	if (ret) {
		printf("ddr: get sdram-width failed!\n");
		return ret;
	}
	sdram_width = val;
	switch (val)
	...
	case 16:
		...
		break;
	default:
		printf("ddr: sdram width error!\n");
		return -EINVAL;
	}

	ret = dev_read_u32(dev, "sdram-banks", &val);
	if (ret) {
		printf("ddr: get sdram-banks failed!\n");
		return ret;
	}
	sdram_banks = val;
	switch (val)
	...
	case 8:
		...
		break;
	default:
		printf("ddr: sdram banks error!\n");
		return -EINVAL;
	}

	ret = dev_read_u32(dev, "sdram-bank-groups", &val);
	if (ret) {
		printf("ddr: get sdram-bank-groups failed!\n");
		return ret;
	}
	sdram_bank_groups = val;
	switch (val)
	{
	case 0:
		mc->sdram_bank_groups = MC_SDRAM_BANK_GROUPS_0;
		break;
	case 2:
		mc->sdram_bank_groups = MC_SDRAM_BANK_GROUPS_2;
		break;
	case 4:
		mc->sdram_bank_groups = MC_SDRAM_BANK_GROUPS_4;
		break;
	default:
		printf("ddr: sdram-bank-groups error!\n");
		return -EINVAL;
	}

	ret = dev_read_u32(dev, "sdram-rows", &val);
	if (ret) {
		printf("ddr: get sdram-rows failed!\n");
		return ret;
	}
	sdram_rows = val;
	...
	if (ret) {
		printf("ddr: get sdram-cols failed!\n");
		return ret;
	}
	sdram_cols = val;
	...
	if (dev_read_bool(dev, "addr-mirror"))
		mc->addr_mirror = MC_ADDR_MIRROR_YES;

	if (dev_read_bool(dev, "reset-revert"))
		mc->reset_revert = true;

	if (dev_read_bool(dev, "early-printf"))
		mc->early_print = MC_EARLY_PRINT_YES;

	if (dev_read_bool(dev, "ddr-param-store"))
		mc->ddr_param_store = MC_DDR_PARAM_STORE_YES;
	...
	ret = dev_read_u32(dev, "mc-dqs-odt", &val);
	if (!ret)
		mc->mc_dqs_odt = val;
	else
		mc->mc_dqs_odt = 6;
	...
	sdram_size = (1 << sdram_rows) * (1 << sdram_cols)
					* sdram_banks * sdram_bank_groups * (sdram_width / 8);
	/*总内存的大小*/
	mc->info.size = sdram_size * (sdram_data_width / sdram_width);
	// printf("sdram_size:%d, size:%d\n", sdram_size, mc->info.size);
...
static int ls_ddr_probe(struct udevice *dev)
{
	...
	// mc->info.size = MC0_MEMSIZE / 4 * 1024 * 1024;
#if defined(CONFIG_SPL_BUILD) || !defined(CONFIG_SPL)
	/* ddr soft rst */
	readl(L2XBAR_CONFIG_BASE_ADDR + 0x11c) &= ~0x1;
	readl(L2XBAR_CONFIG_BASE_ADDR + 0x11c) |= 0x1;

	if (!mc->ddr_param_store ) {
		printf("ddr4 first time init and training: it'll take about 15s, please wait ...\n");
	}

	mm_feature_init(mc);
	ddr4_init(TOT_NODE_NUM, &mm_ctrl_info);
	printf("============ddr4 init and training done!========\n");
#ifdef CONFIG_SOC_LS2K300
