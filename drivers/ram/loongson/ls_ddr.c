...
void mc_init(struct mc_setting *mc)
{
	...
	*pMcRegsBase8 = val8;
	...
	memaddr = PHYS_TO_UNCACHED(HIGH_MEM_WIN_BASE);	// DDR HIGH space
	for (i = 0; i < 8; i++) {
		writeq(test_val[i], (volatile u64 *)(memaddr + (i<<3)));
		if (readq((volatile u64 *)(memaddr + (i << 3))) != test_val[i]) {
	...
	memaddr = PHYS_TO_CACHED(HIGH_MEM_WIN_BASE);	// DDR HIGH space
	for (i = 0; i < 8; i++) {
		writeq(test_val[i], (volatile u64 *)(memaddr + (i<<3)));
		if (readq((volatile u64 *)(memaddr + (i << 3))) != test_val[i]) {
...
static int ls_ddr_probe(struct udevice *dev)
{
	...
	mc->info.base = PHYS_TO_CACHED(0x200000000);
...
static const struct udevice_id ls_ddr_ids[] = {
	...
	{ .compatible = "loongson,ls2p500-ddr", .data = MC_ID_2P500 },
	{ }
};
