...
static int do_loongson_factory_boot(struct cmd_tbl *cmdtp, int flag, int argc, char * const argv[])
{
	int ret = -1;

	const char* booted = bdinfo_get(BDI_ID_FACTORY_BOOTED);

	printf("BDI_ID_FACTORY_BOOTED : %s\n", booted);

	if (booted && booted[0] == '1')
		return 0;
	...
	bdinfo_set(BDI_ID_FACTORY_BOOTED, "1");
	bdinfo_save();
...
U_BOOT_CMD(
	factory_boot,    1,    1,     do_loongson_factory_boot,
	NULL,
	NULL
);

static int do_loongson_factory_clear(struct cmd_tbl *cmdtp, int flag, int argc, char * const argv[])
{
	bdinfo_set(BDI_ID_FACTORY_BOOTED, "0");
	bdinfo_save();
...
U_BOOT_CMD(
	factory_clear,    1,    1,     do_loongson_factory_clear,
	NULL,
	NULL
);
#endif
